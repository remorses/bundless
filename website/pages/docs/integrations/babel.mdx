# Using Babel

You can use babel using the `@bundless/plugin-babel` package

## Example

```ts
// bundless.config.js
const { BabelPlugin } = require('@bundless/plugin-babel')

module.exports = {
    plugins: [
        BabelPlugin({
            babelOptions: {
                plugins: [require('babel-plugin-macros')],
            },
        }),
    ],
}
```

Notice that you will need to pass your babel options explicitly and not using `.babelrc` files

This API design choise has been made because usually .babelrc files contain babel presets that transpile jsx to js but this is already done in bundless, you only need to pass some specific plugins that you need
